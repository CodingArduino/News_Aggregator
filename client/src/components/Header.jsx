    ...
    <header>
      <nav className="fixed top-0 left-0 w-full h-auto bg-gray-800 z-10 flex items-center justify-around">
        ...
        <ul className={active ? "nav-ul flex gap-11 md:gap-14 xs:gap-12 lg:basis-3/6 md:basis-4/6 md:justify-end active" : "nav-ul flex gap-14 lg:basis-3/6 md:basis-4/6 justify-end"}>
          ...
          {/* Language Dropdown */}
          <li className="dropdown-li">
            <Link className="no-underline font-semibold flex items-center gap-2" onClick={() => { setShowLanguageDropdown(!showLanguageDropdown); setShowCountryDropdown(false) }}>
              Language
              <FontAwesomeIcon className={showLanguageDropdown ? "down-arrow-icon down-arrow-icon-active" : "down-arrow-icon"} icon={faCircleArrowDown} />
            </Link>
            <ul className={showLanguageDropdown ? "dropdown p-2 show-dropdown" : "dropdown p-2"}>
              {languages.map((element, index) => {
                return (
                  <li key={index} onClick={() => { setShowLanguageDropdown(!showLanguageDropdown) }}>
                    <Link to={"/language/" + element.iso_2_alpha} className="flex gap-3" type="btn"
                      onClick={() => { setActive(!active) }}>
                      <span>{element.languageName}</span>
                    </Link>
                  </li>
                );
              })}
